...
{ 
	...
	scanf("%d",&Tq);
	int burst[n],arrival[n],pro[n],turn_a_t=0,waiting=0,rtime[n],s=n,i,ct,t,f=0;
	for(i=0;i<n;i++){
		pro[i]=i;
...
printf("\nProcess\t|  arrival Time |   Burst Time  |Completion Time|Turnaround Time|Waiting");
	for(t=0,ct=0;s!=0;)
	{
		...
		{
		s--;
		printf("Pro[%d]\t|\t%d\t|\t%d\t|\t%d\t|\t%d\t|  %d\n",ct+1,arrival[ct],burst[ct],t,t-arrival[ct],t-arrival[ct]-burst[ct]);
